function initDiagnosis() {
  ...
  const results = {
    ...
    flow: {
      ...
      summary: [
        "ページは見られていても、必要な情報を探す途中で迷い、問い合わせ前に止まっている可能性があります。",
      ...
      validation: [
        "広告流入後の行動を見ながら、導線上のどこで迷いが出ているかを確認します。",
      ],
    },
    message: {
      ...
      hypothesis: [
        "価値そのものよりも、比較時に思い出される決め手が不足し、最後の候補に残りきれていない可能性があります。",
      ],
      states: [
        "比較段階で違いが一言で残らない",
        "事例・実績が判断材料になりきっていない",
    ...
    offer: {
      ...
      hypothesis: [
        "納得材料や入口設計が不足し、検討はされても、その場で動く理由が弱くなっている可能性があります。",
      ],
      states: [
        "関心はあるが動かない",
        "条件の納得材料が少ない",
    ...
    target: {
      ...
      checks: ["反応する属性", "問い合わせ内容の質", "訴求対象の広さ"],
      nextFocus: [
        "反応している相手が狙いと合っているか",
        "問い合わせ内容に温度差が出ていないか",
        "成約しやすい層まで届いているか",
      ],
      checks: ["反応する属性", "問い合わせ内容の質", "訴求が広すぎないか"],
      validation: [
    ...
    trust: {
      ...
      summary: [
        "興味は持たれていても、実績・事例・会社情報が不足すると問い合わせ前の安心感が残りません。",
    ...
    cycle: {
      ...
      hypothesis: [
        "集客・導線・訴求のどこで止まっているかが曖昧なまま、止まり位置の仮説が曖昧になっている可能性があります。",
      ],
      states: [
        "施策ごとの学びがつながりにくい",
        "判断が感覚に寄りやすい",
      ...
      nextFocus: [
        "流入・導線・訴求の数字が分かれて見えているか",
        "施策ごとの反応差が判断材料になっているか",
        "止まり位置の仮説がひとつに絞れているか",
      ],
      validation: [
        "広告検証を通じて反応を見る条件を揃え、止まり位置の仮説を整理します。",
      ],
    },
  };
  ...
  function showResult() {
    ...
    renderList("result-next-focus", result.nextFocus);
